...
class BiometricFusionModel(nn.Module):
    ...
    def forward(self, inputs: dict[str, Tensor]) -> Tensor:
        ...
        left_feat  = self.iris_branch(inputs["iris_left"])             # (B, 32)
        right_feat = self.iris_branch(inputs["iris_right"])            # (B, 32)

        # Concatenate and classify
        fused  = torch.cat([fp_feat, left_feat, right_feat], dim=-1)  # (B, 1344)
