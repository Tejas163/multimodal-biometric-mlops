...
def create_folds(parquet_dir: Path, n_folds: int = 5, seed: int = 42) -> list[dict[str, Any]]:
    ...
    log.info("Creating %d folds from %d subjects (%d samples)", n_folds, n_subjects, n_samples)
    ...
    for fold_idx in range(n_folds):
        ...
        log.info(
            "Fold %d: %d train samples (%d subjects), %d val samples (%d subjects)",
            fold_idx + 1,
            len(train_indices),
            len(train_subjects),
            len(val_indices),
...
def train_fold(
    ...
    log.info(
        "Fold %d: trainable=%d / total=%d (%.1f%% frozen)",
        fold_num,
        trainable,
        total,
