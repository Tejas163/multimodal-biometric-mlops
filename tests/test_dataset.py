...
class TestBiometricDataset:
    ...
    def test_label_is_int(self, parquet_path):
        ds = BiometricDataset(parquet_path, active_modalities=["face"])
        item = ds[0]
        assert isinstance(item["label"], int)

    def test_num_classes(self, parquet_path):
        ds = BiometricDataset(parquet_path, active_modalities=["face"])
        assert ds.num_classes == 5

    def test_missing_modality_zero_vector(self, parquet_path):
    ...
    def test_feature_dims_property(self, parquet_path):
        ds = BiometricDataset(parquet_path, active_modalities=["face", "voice"])
        assert ds.feature_dims == {"face": 512, "voice": 128}

    def test_index_range(self, parquet_path):
        ds = BiometricDataset(parquet_path, active_modalities=["face"])
        for i in [0, 7, 14]:
            item = ds[i]
            assert "face" in item
