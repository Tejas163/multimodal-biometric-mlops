...
from biometric_ml.data.schema import FINGERPRINT_SHAPE, IRIS_SHAPE
...
def load_global_label_map(parquet_dir: Path) -> dict[int, int]:
    ...
    for split in ["train", "val", "test"]:
        ...
        if p.exists():
            ...
            d = table.to_pydict()
            return {sid: lbl for sid, lbl in zip(d["subject_id"], d["label"])}
...
def run_demo(
    ...
    for rank, (label, prob) in enumerate(zip(top_labels, top_probs), 1):
        ...
        print(f"  {colour}#{rank:<4} Subject {subj:>3}  [{label:>3}]  {pct:>8}   {bar}{marker}{RESET}")

    print(f"\n{'─'*60}")
    if correct:
        print(f"{GREEN}{BOLD}  ✓ CORRECT  — Model identified Subject {subject_id} correctly!{RESET}")
    else:
        print(f"{RED}{BOLD}  ✗ INCORRECT — Predicted Subject {predicted_subject}, True Subject {subject_id}{RESET}")
    ...
    print(f"  Confidence in true subject (label {true_label}): {probs[true_label].item()*100:.2f}%")
    print(f"{'─'*60}\n")
